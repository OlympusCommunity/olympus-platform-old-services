...
public class UtilEncrypting implements Serializable {
	...
	public String aesEncript (String cadena, SecurityKeyDTO securityKeyDTO) {
		log.info(Constantes.PASS_ENCRYPT);
		String hashPassword = null;

		try {
			IvParameterSpec ivParameterSpec = new IvParameterSpec(securityKeyDTO.getIvKey().getBytes(Constantes.UTF_8));
			SecretKeySpec secretKeySpec = new SecretKeySpec(generateKey(securityKeyDTO.getIvKey(), securityKeyDTO.getSaltKey()).getEncoded(), Constantes.AES_ENCRIPT);
			Cipher cipher = Cipher.getInstance(Constantes.AES_STRING);
			cipher.init(Cipher.ENCRYPT_MODE, secretKeySpec, ivParameterSpec);
			//hashPassword = new String(cipher.doFinal(cadena.getBytes(Constantes.UTF_8)));
			hashPassword = base64Encode(new String (cipher.doFinal(cadena.getBytes(Constantes.UTF_8))));
		} catch (Exception e) {
